  ...
  while(1){
    ...
    for(int i=0; i<nbins; i++){
      line(histImg, Point(i, histh),
           Point(i, cvRound(histogram.at<float>(i))),
           Scalar(255), 1, 8, 0);
    }
    
    histImg.copyTo(greyframe(Rect(0, 0,nbins, histh))); 


    Mat equalized_image(greyframe.rows,greyframe.cols,greyframe.type(),Scalar(0));
    Mat equalized_image2(greyframe.rows,greyframe.cols,greyframe.type());

    equalizeHist(greyframe,equalized_image2);
